...
class socketIO():
    ...
    def __init__(self, port, uid, ioHandler):
        ...
        self.ovData = ""
        self.ovSamples = 4
        self.generic_getting_data = False
    ...
    def Connect(self):
        ...
        try:
            print "@CyWebSocket -- Trying to accept socket"
            connection,address = sock.accept()
            print "@CyWebSocket -- Socket accepted"
            self.con = connection
            print "> Connected!"
        except:
    ...
    def run(self):
        ...
        while self.socketThreadRunning == True:
            #print '@CyWebSocket -- self.socketThreadRunning = ', self.socketThreadRunning
            if self.io.getOpenvibe() == True:
            ...
            if self.generic == True:
                try:
                    ...
                    #print '@CyWebSocket -- self.con = ', str(self.con)
                    #print '@CyWebSocket -- about to get ready =  select.select([self.con], [], [], 1)'
                    ready = select.select([self.con], [], [], 1)

                    #print '@CyWebSocket -- ready[0] == ', ready[0]
                    
                    if ready[0]:
                        if self.generic_getting_data:
                            clientData  = self.con.recv(1024)
                            
                            true_data = ''
                            for d in clientData:
                                true_data += chr(ord(d))
                            print '@CyWebSocket -- Data Recieved = ', true_data 
                            self.io.onData(0, true_data)
                            
                        else:
                            clientData  = self.con.recv(1024)
    
                            print '@CyWebSocket -- self.io.onGeneric(0)'
                            self.io.onGeneric(0)
                            self.generic_getting_data = True
            ...
            if not self.isHandleShake: 
                ...
                try:
                    self.con.setblocking(0)
                    ready = select.select([self.con], [], [], 1)
                    print '@CyKIT -- in handlshake ready[0] = ', ready[0]
                    if ready[0]:
                        clientData  = self.con.recv(1024)
                        ...
                        #print '@CyWebSocket -- about to onConnect with io with self.uid = ', self.uid
                ...
                try:
                    #print '@CyWebSocket -- Some Data to be read'
                    ready = select.select([self.con], [], [], 0)
                    #print '@CyKIT -- in non ready[0] = ', ready[0]                    
                    if ready[0]:
                        print '@CyWebSocket -- ready[0] ==', ready[0]
                        ...
                        print '@CyWebSocket -- recieved data = ', data
    ...
    def sendData(self, text):
        if self.uid == 0:
            #print '@CyWebSocket -- data to be sent =', text
            self.con.send(text + "\r\n")
        else: 
